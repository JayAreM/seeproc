<?php
	...
	if($count > 0){
		...
		$sql = "SELECT sum(Gross) as Previous FROM infrapayment where infratracking = '" . $infraTracking . "' and TrackingNumber != '".$trackingNumber."'";
		$record = $database->query($sql);
		$data = $database->fetch_array($record);
						...
						<div id = "textArea"  style = "text-indent: 30px;text-align: justify;text-justify: inter-word; padding-bottom:10px; line-height: 16px;font-family:Arial; font-size: 14px;padding-top:10px;width:100%;" >
							<?php 
								...
								if($delay <= 0){
									...
									if($delay >  0){
										$sheet1 .= "<tr>
														<td></td><td></td><td></td><td>Liquidated Damages</td><td >" . number_format($ld,2). "</td>
													</tr>";	
									}
									$sheet1 .= "<tr>
													<td></td><td></td><td ></td><td style = 'text-align:right;'>Total Deduction </td><td style = 'font-weight:bold;text-align:right;border-top:1px solid black;padding:5px 0px;'>" . number_format($deduction,2). "</td>
