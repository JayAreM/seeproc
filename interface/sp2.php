<?php
 	...
 	if(isset($_GET['st'])){
		...
		$status = $database->charEncoder($_GET['st']);
	...
	/*	$sql = "select a.*,b.Description as CategoryName from(select a.*,b.Title, b.Fund as TitleFund from vouchercurrent a left join fundtitles b on a.PR_AccountCode = b.Code 
		...
		$sql = "select a.*,b.Name as OfficeName from(select a.*,b.Description as CategoryName from(select a.*,b.Title, b.Fund as TitleFund from vouchercurrent a left join fundtitles b on a.PR_AccountCode = b.Code 
					where trackingtype ='" . $type . "' and status = '" . $status . "' and " . $case . " group by trackingnumber) a
		...
		while($data = $database->fetch_array($record)){
			...
			if($i == 1){
				...
				$sheet1 .='<tr><td colspan ="100%" style ="text-align:center;font-size:22px;padding:10px;font-family:anton;line-height:18px;">List of Procurement Transactions
								<div style ="font-size:16px;font-family:nor;">(on a particular status below)</div>
							</td></tr>';
				...
				$sheet1 .='<tr><th>Status</th><td><span class ="tdDetails">' . $status . '</span></td></tr>';	
				...
				$sheet1 .='</td></tr>';
				if($office == "All"){
					$sheet1 .='<tr><th></th><th>Office</th><th>Fund</th><th>Quarter</th><th>Category</th><th>Encoded</th><th>TN</th><th style ="text-align:right;">Total Amount </th><th style ="text-align:left;">Status Updated</th></tr>';
				}else{
					$sheet1 .='<tr><th></th><th>Fund</th><th>Quarter</th><th>Category</th><th>Encoded</th><th>TN</th><th style ="text-align:right;">Total Amount </th><th style ="text-align:left;">Status Updated</th></tr>';
				}
...
<title>Procurement List - Status</title>
<style>
	@font-face{
		font-family: NOR;
