...
<style>
	...
	.back{
		...
		width:100%;
		background:url(../images/landscape.png);	
		//background:url(../images/nature1.gif);	
		background-repeat:no-repeat;
		background-size:100% 102% ;
	...
	.td4{
		background-color:rgb(82, 129, 96);
		padding:5px 2px;
	}
	.label7tracker{ 
		font-size: 16px;	
	...
	.loadMore{
		...
		display:inline-block;padding:2px 10px;font-size:14px;color:white;
		background-color: rgb(135, 144, 133);
	}
	.loadMore:hover{
		background-color: rgb(96, 127, 95);
	}	
</style>
	
	<title>2018 Receiver's Gate</title>
	<link rel="icon" href="/city/images/red.png"/> 
	<div id = "menuContainer" style = "display:n1one;">
		<div  >	
			<div style =" color:white;text-align:left;">
				...
				?>
					...
					<td width ="1" style = "">		
						<select id = "dbYear" class = "selectType" onchange = "changeClaimType(this)">
							<!--<option>2020</option>
							<option>2019</option>
							<option>2018</option>	
							<option>2017</option>	
	...
	<div id = "bodyDiv" style = "display:none">
		<?php
			...
			echo "<div style = 'padding-top:40px;border:0px solid silver;width:450px;margin:0 auto;text-align:left;color:grey;'><b>Search Criteria</b>
						 : Adv, TN#, Claimant, *Checknumber, -Officecode
				      </div>
						";
	...
	<div style = "text-align: center;">
		<div id = "encoder" style = "margin:0 auto;margin-top:50px;box-shadow: 0px 0px 60px 1px black; border:1px solid rgb(226, 229, 227); padding:30px;display: inline-block ;background-color:rgba(255, 255, 255,.5); min-width: 720px;">
			<table id = "" border ="0" style= "width:100%;">
				<tr>
					<td colspan="3" style = "text-align:right;">
						<span class = "data1" style = "margin-right:5px;font-size:18px;" >Search TN#</span>
						<input id  ="ok" class = "text3" maxlength="18" style = "width:200px;font-weight: bold;font-family:oswald; padding:2px 5px; font-size: 22px;text-align:center;" maxlength="6" onkeydown="keypressAndWhatClear(this,event,searchTracking,1)"   value = '' />
					</td>
				</tr>
				<tr>
					<td>
						<div id = "doctrackUpdateContainer" style = "min-height: 700px;">
...
/*	sMonth.value  = d.getMonth()+1;
	...
	function searchNow(me,evt){
		...
		if(charCode == 13){
			...
			loader();
			var queryString = "?searchByTransmitalValue=1&field=" + field + "&searchValueReleaser=" + searchValue + "&dbYear=" + dbYear ;
			var container = document.getElementById('bodyDiv'); 
	...
	function saveControl(me){
		...
		if(answer){
			...
			loader();
			var queryString = "?saveControl2018=1&trackingNumber=" + trackingNumber + "&ctrlNo=" + ctrlNo + "&encoded=" + encoded + "&dbYear=" + dbYear ;    
			var container = document.getElementById('doctrackUpdateContainer'); 
	...
	function skipAndSave(me){
		...
		if(answer){
			...
			loader();
			var queryString = "?skipAndSave2018=1&trackingNumber=" + trackingNumber + "&ctrlNo=" + ctrlNo + "&encoded=" + encoded + "&dbYear=" + dbYear ;    
			var container = document.getElementById('doctrackUpdateContainer'); 
	...
	function receiveThis(me){
		
			var dbYear = document.getElementById('dbYear').value;
			var fund = document.getElementById('fund').textContent;
			var arr = me.id.split("*");
			var status = arr[0];
			var trackingNumber = arr[1];
			var value = document.getElementById('receiveAdv').value.trim();
			loader();
			var queryString = "?receive2018=1&trackingNumber=" + trackingNumber + "&status=" + status + "&dbYear=" + dbYear  + "&input=" + value + "&fund=" + fund;
			var container = document.getElementById('doctrackUpdateContainer'); 
			ajaxGetAndConcatenate1(queryString,processorLink,container,"receive2018");
	}
	function highlight(me){
		...
		if(color != "orange"){
			...
			if(status.textContent == "CAO Released"){ //sa cell
	...
	function loadMore(me){
		...
		var key = split[2];
		loader();
		var queryString = "?loadMore2018=1&id=" + id + "&ctr=" + ctr + "&key=" + key + "&dbYear=" + dbYear;
