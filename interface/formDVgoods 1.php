<?php
	...
	if($count > 0){
		...
		if(strlen(trim($accountNumber))) {
			$particulars .= "<br><br>GASOLINE MONITORING ACCOUNT NUMBER : ".$accountNumber;
		}

		$sql = "SELECT ModeOfProcurement FROM vouchercurrent WHERE TrackingNumber = '".$poTrackingNumber."' LIMIT 1";
		$record = $database->query($sql);
		$data = $database->fetch_array($record);
				...
				<tr>
					<td style = "text-align:center;border-right:1px solid black;border-top:1px solid black;height:286px;padding:0px 5px;vertical-align:top;"><!--286-->
						...
						<div style="<?= $displayNone ?>">
							<table border="0" cellpadding="0" style="border-spacing:0px; margin:0px 0px 0px auto; font-size:12px; font-family:Arial;">
								<tr>
									<td colspan="2" style="text-align:right; padding:0px 5px 0px 0px; font-weight:bold; vertical-align:bottom;">Gross</td>
									<td style="text-align:right; font-weight:bold; font-size:14px;"><?= number_format($gross, 2) ?></td>
								</tr>
								<tr>
								...
								<tr>
									<td style="font-weight:bold; padding-top:2px; padding-right:10px; font-size:10px; white-space:nowrap;">TAX BREAKDOWN<?= $notax ?></td>
									<td style="width:0px;"></td>
									<td></td>
								</tr>
								<tr>
									<td colspan="2" style="padding:5px 5px 0px 0px;">
										<table border="0" cellpadding="0" style="border-spacing:0px; font-size:12px; margin:0px 0px 0px auto;"><?= $taxes ?></table>
									</td>
									<td></td>
								</tr>
								<tr>
