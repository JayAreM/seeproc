<?php	
	...
	$sql = "select x.*,
			...
			on y.TrackingNumber = z.TrackingPartner where x.Id > 0 " . $filter3 . $filter4 .  $filter5 . " 
	...
	while($data = $database->fetch_array($record)){
		...
		if($flag > 0){
			...
			if($lastName != $name){
				...
				$sheet1 = '<tr class ="summary1"><td colspan ="100%" style ="border-top:1px solid white;border-bottom:1px solid white;background-color:rgb(188, 207, 218);padding:20px;padding-right:30px;">
	...
	if($lastCost > 0){
		...
		$sheet .= '<tr class = "summary1"><td colspan ="100%" style ="border-top:1px solid white;border-bottom:1px solid white;background-color:rgb(188, 207, 218);padding:20px;padding-right:30px;">
						  	<table border ="0" style ="float:right;border-spacing:0;width:80%;">
						  		<tr>
						  			<th style ="text-align:left;padding-right:10px;">Project Name :</th>
    ...
    if($totalAllcost > 0 ){
		...
		$sheet .= '<tr class = "summary1"><td colspan ="100%" style ="background-color:rgb(32, 44, 48);padding:20px;">
					<table style ="text-align:right;float:right;color:white;font-Size:14px;border-spacing:0;" border ="0">
						<tr>
							<td colspan ="100%" style ="text-align:left;border-bottom:1px solid grey;font-size:28px; font-family:anton;text-shadow:0px 0px 5px black;">DATA STATISTICS</td>
						</tr>
						<tr>
