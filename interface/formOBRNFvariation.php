...
<?php
	...
	// $sql = "select Variation from infra where trackingnumber = '" . $trackingNumber . "' limit 1";
	// $record = $database->query($sql);
	// $data = $database->fetch_array($record);
	// $net = $data['Variation'];
	$sql = "select Variation1,VariationObr from infra where trackingnumber = '" . $trackingNumber . "' limit 1";
	$record = $database->query($sql);
	$data = $database->fetch_array($record);
	// $net = $data['Variation1'];
	$net = $data['Variation'];
	if($data['Variation1'] > 0) {
		$net = $data['Variation1'];
	}
	$obr = $data['VariationObr'];
