<?php
 	...
 	if(isset($_GET['office'])){
		...
		if($type == 1){
			...
			from(select * from vouchercurrent where Trackingtype = 'PO' and " .  $case1 ." and Claimant = '" . $sup . "'  and status != 'Cancelled' group by trackingnumber ) a
			left join ppmpcategories b on a.PR_CategoryCode = b.code
			left join office c on a.office = c.code 
			 order by c.Name asc,a.Id asc";	
		}else if($type == 2){
			$sql = "select a.*,b.Description as CategoryName ,c.Name as OfficeName
			from(select * from vouchercurrent where Trackingtype = 'PO' and " .  $case1 ." and Claimant = '" . $sup . "' and status = 'Waiting for Delivery' group by trackingnumber ) a
			left join ppmpcategories b on a.PR_CategoryCode = b.code
			left join office c on a.office = c.code 
			 order by c.Name asc,a.Id asc";	
		}else if($type == 3){
			$sql = "select a.*,b.Description as CategoryName ,c.Name as OfficeName
			from(select * from vouchercurrent where Trackingtype = 'PO' and " .  $case1 ." and Claimant = '" . $sup . "' and status != 'Waiting for Delivery' and status != 'Cancelled' group by trackingnumber ) a
			left join ppmpcategories b on a.PR_CategoryCode = b.code
			left join office c on a.office = c.code 
			 order by c.Name asc,a.Id asc";	
		}
		...
		while($data = $database->fetch_array($record)){
			...
			if($i == 1){
				...
				$sheet1 .='<tr><th>Supplier</th><td ><span class ="tdDetails" style ="white-space:nowrap;">' . utf8_encode(stripslashes($sup)) . '</span></td></tr>';
				
				if($type == 2){
					$sheet1 .='<tr><th>Status</th><td><span class ="tdDetails">Waiting for Delivery</span></td></tr>';
				}else{
					$sheet1 .='<tr><th>Status</th><td><span class ="tdDetails">Mixed</span></td></tr>';
				}
				...
				$sheet1 .='<th>Quarter</th><th>Fund</th>
							<th>Category</th>
						   <th>Encoded</th><th>PO Number</th><th>TN</th>
