 	...
 	if(isset($_GET['office'])){
		...
		if($type == 1){
			$sql = "select a.*, b.Description as CategoryName,c.Name as OfficeName from(SELECT  Id,ADV1,Status,DateEncoded,DateModified,Year,Office,Fund,TotalAmountMultiple,Amount,PO_Number,PR_CategoryCode,PR_Month,TrackingNumber, Claimant
					FROM citydoc2023.vouchercurrent where TrackingType = 'PX' and  " . $case1 . "  and Status != 'Cancelled' group by TrackingNumber ) a
			...
			$sql = "select a.*, b.Description as CategoryName,c.Name as OfficeName from(SELECT  Id,ADV1,Status,DateEncoded,DateModified,Year,Office,Fund,TotalAmountMultiple,Amount,PO_Number,PR_CategoryCode,PR_Month,TrackingNumber, Claimant
					FROM citydoc2023.vouchercurrent where TrackingType = 'PX' and  " . $case1 . "  and  Status = 'Check Released' group by TrackingNumber) a
					left join ppmpcategories b on a.PR_CategoryCode = b.code
			...
			$sql = "select a.*, b.Description as CategoryName,c.Name as OfficeName from(SELECT  Id,ADV1,Status,DateEncoded,DateModified,Year,Office,Fund,TotalAmountMultiple,Amount,PO_Number,PR_CategoryCode,PR_Month,TrackingNumber, Claimant 
					FROM citydoc2023.vouchercurrent where TrackingType = 'PX' and  " . $case1 . "  and Status != 'Cancelled' and Status != 'Check Released' group by TrackingNumber) a
					left join ppmpcategories b on a.PR_CategoryCode = b.code
					left join office c on a.office = c.code 
					order by a.Claimant asc ";
