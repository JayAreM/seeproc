<?php
	...
	if($count > 0) {
		...
		if($oneTax == 2) {
			...
			$record = $database->query($sql);
			$taxRows = $database->num_rows($record);
			...
			$percentageAmount1 = $data['PercentageAmount'];
			$codeType1 = $data['CodeType'];
			if($codeType1 == 'Expanded') {
				$codeType1 = 'EXP';
			}

			$compBase = number_format( ($amount / 1.12) , 2 );
			if($receiptType == 'NON-VAT') {
				$compBase = number_format($amount);
			}
			if($oneTax == 1) {
				$compDisp1 = "(<strong>".number_format($baseAmount, 2)."</strong>) &#10006; ".$percentage."%";
			}else {
				$compDisp1 = number_format($amount, 2)." / 1.12 (<strong>".$compBase."</strong>) &#10006; ".$percentage."%";
				if($receiptType == 'NON-VAT') {
					$compDisp1 = "(<strong>".$compBase."</strong>) &#10006; ".$percentage."%";
				}
			}


			$totalTax += $percentageAmount1;
			...
			if($taxRows > 1) {
				...
				$percentageAmount2 = $data['PercentageAmount'];
				$codeType2 = $data['CodeType'];

				if($codeType2 == 'Expanded') {
					$codeType2 = 'EXP';
				}
				...
				$compDisp2 = number_format($amount, 2)." / 1.12 (<strong>".$compBase."</strong>) &#10006; ".$percentage."%";
				if($receiptType == 'NON-VAT') {
					$compDisp2 = "(<strong>".$compBase."</strong>) &#10006; ".$percentage."%";
				}

				$totalTax += $percentageAmount2;
		...
		$sql = "SELECT count(Id) as cnt FROM listattachments WHERE TrackingNumber = '".$trackingNumber."'";
		$record = $database->query($sql);
		$data = $database->fetch_array($record);
					...
					<td style = "text-align:center;border-right:1px solid black;border-top:1px solid black;height:286px;padding:0px 5px;vertical-align:top;"><!--286-->
						...
						?>
							<table border="0" cellpadding="0" id="taxBreakdown" style="border-spacing:0px; margin:0px 0px 0px auto; font-size:12px; font-family:Arial;">
								...
								<tr>
									<td style="font-weight:bold; padding-top:2px; padding-right:10px; font-size:10px; white-space:nowrap;">TAX BREAKDOWN<?= $recTypeLabel ?></td>
									<td style="width:0px;"></td>
									<td></td>
								</tr>
								<tr>
									<td colspan="2" style="padding:5px 5px 0px 0px;">
										<table border="0" cellpadding="0" id="taxesCont" style="border-spacing:0px; font-size:12px; margin:0px 0px 0px auto;">
										<?php
											if($percentageAmount1 > 0) {
										?>
											<tr>
												<td style="padding-right:5px;"><?= $codeType1 ?></td>
												<td style="padding-right:5px;"><?= $compDisp1 ?></td>
												<td style="text-align:right; padding:0px 95px 0px 5px;"><?= number_format($percentageAmount1, 2) ?></td>
											</tr>
										<?php
											}
										?>
										<?php
											if($percentageAmount2 > 0) {
										?>
											<tr>
												<td style="padding-right:5px;"><?= $codeType2 ?></td>
												<td style="padding-right:5px;"><?= $compDisp2 ?></td>
												<td style="text-align:right; padding:0px 95px 0px 5px;"><?= number_format($percentageAmount2, 2) ?></td>
											</tr>
										<?php
											}
										?>
								...
								<?php
									if(strlen(trim($adjustmentLabel)) > 0) {
								?>
								<tr>
									<td></td>
									<td style="padding:0px 5px 0px 0px; white-space:nowrap;"><span style="font-weight:bold; font-size:10px;"><?= $adjustmentType ?> :</span> <?= $adjustmentLabel ?></td>
									<td style="text-align:right; padding:0px 3px 0px 8px;"><?= number_format($adjustmentAmount, 2) ?></td>
								</tr>
								<?php
									}
								?>
