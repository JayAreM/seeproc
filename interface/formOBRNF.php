...
<?php
	/*$sql = "SELECT a.*,b.Name,c.Address,d.Name as ProgramName,e.Title, f.OBRParticulars
								
								FROM vouchercurrent a 
								
								left join office b on a.office = b.code  
								
								left join (select Name,Address  from supplier.supplierinfo group by Name ) c on a.Claimant = c.Name  
								
								left join programcode d on a.PR_ProgramCode = d.code
								left join fundtitles e on a.PR_AccountCode = e.Code
								
								left join particulars f on a.TrackingNumber = f.TrackingNumber
								where a.trackingnumber  = '" . $trackingNumber . "' order by pr_programcode,pr_accountcode";
	
	$record = $database->query($sql);*/
	//echo $sheet->CreateFormOBR($record,$trackingNumber,$officeName,$payee);
	...
	$sql = "select Year,PR_ProgramCode,Claimant,NetAmount,OBR_Number from vouchercurrent where TrackingNumber = '" . $trackingNumber . "' limit 1";
	$record = $database->query($sql);
	...
	if(strlen($batchNo) > 8 ){
		...
		if($count > 1){
			...
			$sql = "select a.TrackingNumber, a.FundYear,NetAmount,PR_ProgramCode,PR_AccountCode,InfraId,b.Title,c.Name from  
					vouchercurrent a left join FundTitles b on a.PR_AccountCode = b.Code 
					left join programcode c on a.PR_ProgramCode = c.Code
					where a.TrackingNumber in("  . $tns . ") order by TrackingNumber asc";
			$record  = $database->query($sql);
			...
			$expenseCodes ='';
			while($data = $database->fetch_array($record)){
				array_push($nfTns, $data['TrackingNumber']);
			...
			$sql = "SELECT * FROM programcode WHERE TrackingNumberInfra IN (".$tns.") ORDER BY TrackingNumberInfra ASC";
			$record  = $database->query($sql);
			$projectTitles = [];
			while($data = $database->fetch_array($record)){
				array_push($projectTitles, $data['Name']);
			}
	...
	for($i = 0; $i < sizeof($amount);$i++){
		...
		if($header != $programCode[$i] || $yearHeader != $fundYear[$i]){
			$fYear = '';
			...
			if($forBatchDisplay == 1) {
				$h3 .= '	<td colspan = "3" style = "text-align:left;font-weight:bold;border-top:2px solid black;border-right:2px solid black;padding:2px;padding-left:15px;">
								'  . $fYear . ' ' . $programCode[$i] . '
								<span style = "font-weight:normal;"> ' . $programName[$i] . '</span>
								<div style = "font-weight:normal;"><span style="font-weight:bold;">Tracking Number: '.$nfTns[$i].'</span></div>
							</td>';
			}else {
				$h3 .= '	<td colspan = "3" style = "text-align:left;font-weight:bold;border-top:2px solid black;border-right:2px solid black;padding:2px;padding-left:15px;">
								'  . $fYear . ' ' . $programCode[$i] . '
			...
			$yearHeader = $fundYear[$i];
		}
		$h3 .= '<tr style ="font-size:12px;">';

		// if($forBatchCount == 1) {
		// 	$h3 .= '	<td colspan = "3"  style ="text-align:left;border-top:2px solid black;border-right:2px solid black;border-top:2px solid black;padding:0">'.$projectTitles[$i].'</td>';
		// }else {
