<?php
 	...
 	if(isset($_GET['office'])){
		...
		$sup = utf8_decode($database->charEncoder($_GET['sup']));
		...
		if($type == 1){
			$sql = "select a.*, b.Description as CategoryName,c.Name as OfficeName from(SELECT  Id,ADV1,Status,DateEncoded,DateModified,Year,Office,Fund,TotalAmountMultiple,Amount,PO_Number,PR_CategoryCode,PR_Month,TrackingNumber, Claimant
					FROM citydoc2023.vouchercurrent where TrackingType = 'PX' and  " . $case1 . " and Claimant = '" . $sup . "' and Status != 'Cancelled' group by TrackingNumber ) a
					left join ppmpcategories b on a.PR_CategoryCode = b.code
					left join office c on a.office = c.code order by a.Claimant asc
					";
			$statusLabel = "Mixed";	
		}else if($type == 2){
			$sql = "select a.*, b.Description as CategoryName,c.Name as OfficeName from(SELECT  Id,ADV1,Status,DateEncoded,DateModified,Year,Office,Fund,TotalAmountMultiple,Amount,PO_Number,PR_CategoryCode,PR_Month,TrackingNumber, Claimant
					FROM citydoc2023.vouchercurrent where TrackingType = 'PX' and  " . $case1 . " and Claimant = '" . $sup . "' and  Status = 'Check Released' group by TrackingNumber) a
					left join ppmpcategories b on a.PR_CategoryCode = b.code
					left join office c on a.office = c.code 
					order by a.Claimant asc";
			$statusLabel = "Check Released";
		}else if($type == 3){
			$sql = "select a.*, b.Description as CategoryName,c.Name as OfficeName from(SELECT  Id,ADV1,Status,DateEncoded,DateModified,Year,Office,Fund,TotalAmountMultiple,Amount,PO_Number,PR_CategoryCode,PR_Month,TrackingNumber, Claimant 
					FROM citydoc2023.vouchercurrent where TrackingType = 'PX' and  " . $case1 . " and Claimant = '" . $sup . "' and Status != 'Cancelled' and Status != 'Check Released' group by TrackingNumber) a
					left join ppmpcategories b on a.PR_CategoryCode = b.code
					left join office c on a.office = c.code 
					order by a.Claimant asc ";
		...
		while($data = $database->fetch_array($record)){
			...
			$adv = $database->zeroToNothing($data['ADV1']);
			...
			if($i == 1){
				...
				$sheet1 .='<tr><td colspan ="100%" style ="text-align:center;font-size:22px;padding:10px;font-family:anton;line-height:18px;">List of Procurement Transactions
								<div style ="font-size:16px;font-family:nor;">(base on supplier payment)</div>
							</td></tr>';
				...
				$sheet1 .='<tr><th>Transaction</th><td><span class ="tdDetails">Payment</span></td></tr>';
			
				$sheet1 .='<tr><th>Supplier</th><td><span class ="tdDetails">'  . $sup . '</span></td></tr>';
			...
			$sheet2 .= '<td class ="tdData" style ="text-align:center;">' . $adv . '</td>';
...
<title>Procurement List - Payments</title>
<style>
	@font-face{
		font-family: NOR;
