<?php
	...
	if($count > 0){
		...
		$sql = "SELECT Amount, Total, TotalLD, LiquidatedDamages FROM pxrecord WHERE TrackingNumber = '".$trackingNumber."'";
		$record = $database->query($sql);
		$gross = 0;
		$grossWLD = 0;
		$totalLDlang = 0;
		while ($data = $database->fetch_array($record)) {
			...
			$ldLang = floatval($data['LiquidatedDamages']);
			$totalLDlang += $ldLang;
			$gross += $totalPX;
			// $grossWLD += $totalLD;
		}
		$grossWLD = $gross - $totalLDlang;
		...
		$record = $database->query($sql);
		$data = $database->fetch_array($record);
		$particulars = nl2br($data['Particulars']);
		...
		if(strlen(trim($particulars)) == 0) {
			...
			$particulars = "TO PAYMENT FOR ".$categoryName." OF THE ".$officeName." UNDER P.O# <strong>".$poNumber."</strong> DATED <strong>".$poDate."</strong> WITH INV# <strong>".$invoiceNumber."</strong>&nbsp;&nbsp;DATED:&nbsp;<strong>".$invoiceDate."</strong> AS PER SUPPORTING DOCUMENTS HERETO ATTACHED.";
		}

		$sql = "SELECT ModeOfProcurement, NatureOfPayment, PaymentTerm FROM vouchercurrent WHERE TrackingNumber = '".$poTrackingNumber."' LIMIT 1";
		$record = $database->query($sql);
		$data = $database->fetch_array($record);
		...
		if($poMode == 5 || $poMode == 12 || $poMode == 18) {
			if($pyTerm != 4) {
		...
		if($poMode == 18) {
			$particulars = "TO PAYMENT OF OFFICE SUPPLIES & MATERIALS FOR THE CITY GOVERNMENT OF DAVAO (DBM-PS) AS PER PERTINENT PAPERS HERETO ATTACHED AMOUNTING TO...";
			// $noTaxNote = "";
			$displayNone = "visibility:hidden;";
			$noTaxGross = number_format($gross, 2);
			$charges = "";
		...
		// 	$natureOfTranStr = "NATURE OF TRANSACTION : ".$nature;
		// }
			...
			<table style = "border-spacing:0;border-top:1px solid black;" border = "0">
				...
				<tr>
					<td style = "text-align:center;border-right:1px solid black;border-top:1px solid black;height:286px;padding:0px 5px;vertical-align:top;"><!--286-->
						<div id = "textArea"  style = "text-indent: 30px;text-align: justify;text-justify: inter-word; padding-bottom:10px; line-height: 16px; font-family:Arial; font-size: 12px; padding-top:10px; width:100%; text-transform:uppercase; max-width:552px; word-break:break-word;" >
							<?php 
