<?php	
    ...
    if($totalAllcost > 0 ){
		...
		$sheet .= '<tr  class = "summary1" style ="">
							<td colspan ="100%" style ="padding-top:40px;padding-left:20px;text-align:left;font-size:28px; font-family:anton;color:rgb(32, 44, 48);">FUND USAGE RATIO
							</td>
						</tr>';	   
		...
		foreach($arrayOffice as $val => $x){
			$col =  $val ;
			$pie2Data .= $arrayOffice[$col]["Name"] . '*' . $arrayOffice[$col]["Fund"] . '*' .  $arrayOffice[$col]["PR"] . '*' . $arrayOffice[$col]["PRcount"] . '*' . $arrayOffice[$col]["PO"] . '!';
		}		   
		$sheet .= '<tr class = "summary1">
						<td colspan ="100%" style = "padding-top:20px;">
							<table border ="0">
...
<script>
	...
	function bars(){
		...
		pieTable += "<tr style = 'font-size:12px;font-weight:bold;' ><td></td><td></td><td style ='padding-left:10px;padding-bottom:10px;'>Implementing Office</td>" + 
						"<td style ='padding:0px 5px;padding-bottom:10px;text-align:right;'>Total Fund</td>" +
						"<td style ='padding:0px 5px;padding-bottom:10px;text-align:center;'>PR</td>" +
						"<td style ='padding:0px 5px;padding-bottom:10px;text-align:right;'>Amount</td>" +
						"<td style ='padding:0px 5px;padding-bottom:10px;text-align:right;padding-left:50px;'>%</td>" +
						"<td style ='padding:0px 5px;padding-bottom:10px;font-weight:normal;'>from the total fund</td>" +
						"</tr>";
		var sumCount = 0;
		var sumAmount = 0;
		for(var i = 0; i < arr1.length-1; i++){
			...
			var percent =parseFloat((pr / fund) * 100).toFixed(2);
			...
			pieTable += "<tr>" + 
							"<td style = 'text-align:right;font-size:12px;padding:0px 5px;'>" +  (i+1) + "</td>" +
						 	...
						 	"<td style ='text-align:center;'>" + prCount + "</td>" +
						 	"<td style ='text-align:right;padding-right:5px;'>" + numberWithCommas(pr) + "</td>" +
